...
import { envelopeSchema, SUPPORTED_VERSION } from './envelope.js';
...
export class EventConsumer {
    ...
    async handleMessage(message) {
        ...
        try {
            ...
            logger.error({ err: error, body: message.content.toString('utf8').slice(0, 500) }, 'Malformed envelope; dead-lettering.');
        ...
        if (envelope.version !== SUPPORTED_VERSION) {
            log.error({ version: envelope.version, supported: SUPPORTED_VERSION }, 'Unsupported event version; dead-lettering.');
            channel.nack(message, false, false);
            return;
        }
        // ---- Unknown type ----------------------------------------------------
