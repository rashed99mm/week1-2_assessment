...
export interface QueueEmailInput {
  ...
  attachments?: { filename: string; cid: string; content: string }[]
...
export async function queueEmail(input: QueueEmailInput): Promise<void> {
  try {
    ...
    if (isDuplicate(error)) {
      logger.debug(
        { sourceEventId: input.sourceEventId, template: input.template },
        'Email already queued for this event; skipping.',
      )
      return
    }
 ...
 * rest of the batch.
 */
export async function flushOutbox(limit = 25): Promise<{ sent: number; failed: number }> {
  ...
  for (const row of due) {
    try {
      const html = await render(row.template, row.context as Record<string, unknown>)
      ...
      if (row.attempts >= config.EMAIL_MAX_ATTEMPTS) {
        ...
        row.status = 'failed'
        logger.error(
          { to: row.to, template: row.template, attempts: row.attempts },
          'Giving up on an email after repeated failures.',
        )
      } else {
...
export function startEmailFlusher(): void {
...
export function stopEmailFlusher(): void {
  if (flusherTimer) {
