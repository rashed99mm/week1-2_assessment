 ...
 *
 * Separated from main.ts so tests can drive it with `app.inject()` without
 * binding a port or connecting to a broker.
 */
/**
 * The concrete instance type, with the logger generic bound by
 * `loggerInstance`. Annotating the return as a bare `FastifyInstance` would
 * widen the logger back to `FastifyBaseLogger` and fail to assign.
 */
...
export async function buildApp() {
  ...
  app.setNotFoundHandler((_request, reply) =>
    reply.code(404).send(fail('Not found.', 404)),
  )
